...
RETURNING ID into ${bind}`;
  ...
  test('should support multiple binding with a name', {
    concurrency: true,
  }, (t: test.TestContext) => {
...
where FIELD_ID = ${fieldId}`;
  ...
  describe('method Join', () => {
    ...
    test('should equal empty when joining an empty array', {
      concurrency: true,
    }, (t: test.TestContext) => {
      t.assert.deepStrictEqual(sql`,`.join([]), empty);
    });

    test('should allow joining multiple sql queries together', {
      concurrency: true,
    }, (t: test.TestContext) => {
      const queries = [
        `select * from table_1;`,
    ...
    test('should join multiple sql strings together with variables', {
      ...
      const statements = [sql`one = ${1}`, sql`two = ${2}`, sql`three = ${3}`];
